  ...
  return `
    ...
    <body>
      <div class="header">
        ...
        <p style="margin: 10px 0 0 0; color: #6b7280;">Submitted by: ${submittedBy} • ${new Date(submittedAt).toLocaleString()}</p>
      </div>
      ...
      <div class="section">
        ...
        ${measurement.map(item => `<div class="list-item">${item}</div>`).join('')}
      </div>
      ...
      ${exampleLinks?.length ? `
        <div class="section">
          ...
          ${exampleLinks.map(link => `<div class="list-item"><a href="${link}" target="_blank">${link}</a></div>`).join('')}
        </div>
      ` : ''}
      ...
      ${activityType === 'broader-campaign' ? `
        <div class="campaign-details">
          ...
          ${preferredChannels?.length ? `
            ...
            ${preferredChannels.map(channel => `<div class="list-item">${channel}</div>`).join('')}
          ` : ''}
